...
def test_missing_skill_source_is_rejected(tmp_path: Path) -> None:
    manifest = tmp_path / "install" / "manifests"
    manifest.mkdir(parents=True)
    (manifest / "host.json").write_text(
        '{"schema_version":1,"host":"h","adapter_module":"m","skill_source":"skills/missing",'
        '"skill_target":".claude/skills/spec-driven-development",'
        '"settings_target":".claude/settings.json",'
        '"settings_fragment":{"hooks":{"SessionStart":[]}}}', encoding="utf-8"
    )
    assert "host manifest skill_source missing: host.json" in validate_release(tmp_path)
